...
def player_rest(name: str) -> (bool, list):
    ...
    if data_management.search_cache_battle_by_player(name) is None:
        player_inst.recover()
        data_management.update_player_info(player_inst.name)
        return True, [f"{name}, você descansou e recuperou HP, Chakra e Stamina."]
...
def show_area(name: str) -> (bool, list):
    ...
    return True, [f"You are currently in **{area_inst.name}** (Area {area_inst.number}). Rank mínimo: **{area_inst.min_rank}**. Boss: **{area_inst.boss}**."]
...
def travel_to_area(name: str, area_number: int) -> (bool, list):
    ...
    area_inst = data_management.search_cache_area_by_number(area_number)
    if not area_inst.player_can_access(player_inst.ninja_rank):
        return False, [f"Rank insuficiente. Requer: {area_inst.min_rank}."]

    player_inst.current_area = area_number
    data_management.update_player_info(player_inst.name)
    return True, [f"You have traveled to {area_inst.name}."]
...
def set_ninja_identity(name: str, clan: str, element: str) -> (bool, list):
    """Sets player's clan and element after Academy registration."""
    player_inst = data_management.search_cache_player(name)
    if player_inst is None:
        return False, [ERROR_CHARACTER_DOES_NOT_EXIST]
    if data_management.search_cache_battle_by_player(name) is not None:
        return False, [ERROR_CANNOT_DO_WHILE_IN_FIGHT]
    if player_inst.set_ninja_identity(clan, element):
        data_management.update_player_info(player_inst.name)
        return True, messager.empty_queue(name)
    return False, ["Clã ou elemento inválido. Escolha opções válidas (não 'Nenhum' para ambos)."]


def show_missions(name: str) -> (bool, list):
    """Lists missions available for the player's rank."""
    player_inst = data_management.search_cache_player(name)
    if player_inst is None:
        return False, [ERROR_CHARACTER_DOES_NOT_EXIST]
    if data_management.search_cache_battle_by_player(name) is not None:
    ...
    if not missions:
        return True, ["Nenhuma missão disponível para seu rank atual."]

    msg_lines = [f"**Rank atual**: {player_inst.ninja_rank}\n"]
    for m in missions:
        msg_lines.append(
            f"**[{m.rank}] {m.name}** — Stamina: {m.stamina_cost} | "
            f"XP: {m.xp_reward} | Ryo: {m.ryo_reward}\n_{m.description}_\n"
        )
    return True, ["\n".join(msg_lines)]


def accept_mission(name: str, mission_name: str) -> (bool, list):
    ...
    mission_inst = data_management.search_cache_mission_by_name(mission_name)
    if mission_inst is None:
        return False, ["Missão não encontrada."]

    ok, result = mission_module.accept_mission(player_inst, mission_inst)
    if not ok:
        return False, [result]
    data_management.update_player_info(player_inst.name)
...
def start_dungeon(name: str, dungeon_name: str) -> (bool, list):
    ...
    if not dungeon_inst.player_can_enter(player_inst.ninja_rank, player_inst.lvl):
        return False, [f"Requisitos não atendidos. Rank mínimo: {dungeon_inst.min_rank}, Nível rec.: {dungeon_inst.recommended_lvl}."]
